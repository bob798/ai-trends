...
- **`themes/`** — 多条 signal 聚成的主题分析。signal 是点，theme 是连接点的线。
- **`_thinking/`** — 思考考古。重大节点（如本仓库的启动、某个 theme 第一次成形）保留"当时是从什么问题出发、走了哪几步、为什么"。不是日志，不是计划，是**问题驱动的过程记录**。
- **`prompt/`** — 一手原始材料存档。值得逐字研读的系统提示词（system prompt）原文及其中文翻译，不做拆解、不做概括。
